...
for h = [0.001, 0.01, 0.1]
    ...
    for i = 1:4
        [x, y] = ode_am(f{i}, y0(i), range{i}, h,1);
        subplot(4, 4, i)
        plot(x, y, 'r')
        hold on
        fplot(F{i}, range{i}, 'b')
        ...
        legend('Adams-Moulton', 'Funkcja')
        hold off
    end
    % Rzad drugi metody
    for i = 1:4
        [x, y] = ode_am(f{i}, y0(i), range{i}, h,2);
        subplot(4, 4, i+4)
        plot(x, y, 'r')
        hold on
        fplot(F{i}, range{i}, 'b')
    ...
    % Rzad trzeci metody
    for i = 1:4
        [x, y] = ode_am(f{i}, y0(i), range{i}, h,3);
        subplot(4, 4, i+8)
        plot(x, y, 'r')
        hold on
        fplot(F{i}, range{i}, 'b')
    ...
    % Rzad czwarty metody
    for i = 1:4
        [x, y] = ode_am(f{i}, y0(i), range{i}, h,4);
        subplot(4, 4, i+12)
        plot(x, y, 'r')
        hold on
        fplot(F{i}, range{i}, 'b')
